...
<script setup>
    ...
    const emit = defineEmits(['onStatus', 'update:checked', 'toggle']);
    ...
    const ballStatus = ref(Boolean(props.checked));
    watch(() => props.checked, (next) => {
        ballStatus.value = Boolean(next);
    });
    ...
    const ifTwinkle = (id) => {
        return twinkleList.some(twinkleStar => twinkleStar.twinkleShowStatus && id === twinkleStar.twinkleId);
    };
